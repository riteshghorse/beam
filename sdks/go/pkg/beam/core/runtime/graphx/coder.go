...
// Add adds the given coder to the set and returns its id. Idempotent.
func CoderToUrn(c *coder.Coder) (string, error) {
	switch c.Kind {
	case coder.Custom:
		return urnCustomCoder, nil

	case coder.KV:
		return urnKVCoder, nil

	case coder.Nullable:
		return urnNullableCoder, nil

	case coder.CoGBK:
		return urnCoGBKList, nil

	case coder.WindowedValue:
		return urnWindowedValueCoder, nil

	case coder.Bytes:
		return urnBytesCoder, nil

	case coder.Bool:
		return urnBoolCoder, nil

	case coder.VarInt:
		return urnVarIntCoder, nil

	case coder.Double:
		return urnDoubleCoder, nil

	case coder.String:
		return urnStringCoder, nil

	case coder.Row:
		return urnRowCoder, nil

	case coder.Timer:
		return urnTimerCoder, nil

	case coder.Iterable:
		return urnIterableCoder, nil
