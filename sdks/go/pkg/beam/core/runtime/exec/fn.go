...
type timerProvider struct {
	...
	codersByKey  map[string]*coder.Coder // TODO(riteshghorse): do we need this as the coder is same for every key?
}
...
func (t *timerProvider) Set(data timers.TimerMetadata) error {
	...
	//TODO(riteshghorse): do we need to store the set timer here?
	return nil
}
...
func Invoke(ctx context.Context, pn typex.PaneInfo, ws []typex.Window, ts typex.EventTime, fn *funcx.Fn, opt *MainInput, bf *bundleFinalizer, we sdf.WatermarkEstimator, ta UserTimerAdapter, dm DataManager, extra ...interface{}) (*FullValue, error) {
	if fn == nil {
		return nil, nil // ok: nothing to Invoke
	}
	inv := newInvoker(fn)
	return inv.Invoke(ctx, pn, ws, ts, opt, bf, we, ta, dm, extra...)
}

// InvokeWithoutEventTime runs the given function at time 0 in the global window.
func InvokeWithoutEventTime(ctx context.Context, fn *funcx.Fn, opt *MainInput, bf *bundleFinalizer, we sdf.WatermarkEstimator, ta UserTimerAdapter, dm DataManager, extra ...interface{}) (*FullValue, error) {
	if fn == nil {
		return nil, nil // ok: nothing to Invoke
	}
	inv := newInvoker(fn)
	return inv.InvokeWithoutEventTime(ctx, opt, bf, we, ta, dm, extra...)
}
...
func (n *invoker) InvokeWithoutEventTime(ctx context.Context, opt *MainInput, bf *bundleFinalizer, we sdf.WatermarkEstimator, ta UserTimerAdapter, dm DataManager, extra ...interface{}) (*FullValue, error) {
	return n.Invoke(ctx, typex.NoFiringPane(), window.SingleGlobalWindow, mtime.ZeroTimestamp, opt, bf, we, ta, dm, extra...)
}

// Invoke invokes the fn with the given values. The extra values must match the non-main
// side input and emitters. It returns the direct output, if any.
func (n *invoker) Invoke(ctx context.Context, pn typex.PaneInfo, ws []typex.Window, ts typex.EventTime, opt *MainInput, bf *bundleFinalizer, we sdf.WatermarkEstimator, ta UserTimerAdapter, dm DataManager, extra ...interface{}) (*FullValue, error) {
	// (1) Populate contexts
