...
// PipelineTimers returns a list of PipelineTimer objects used to set/reset global pipeline timers (if any).
...
func AsDoFn(fn *Fn, numMainIn mainInputs) (*DoFn, error) {
	...
	// Make sure that all timer entries have keys. If they don't set them to the struct field name.
	if fn.Recv != nil {
		v := reflect.Indirect(reflect.ValueOf(fn.Recv))
		for i := 0; i < v.NumField(); i++ {
			f := v.Field(i)
	...
	if _, ok := fn.Fn.TimerProvider(); ok {
		if len(pt) == 0 {
			...
			return errors.SetTopLevelMsgf(err, "ProcessElement uses a TimerProvider, but no timer struct-tags are attached to the DoFn"+
				", Ensure that you are including the timer structs you're using to set/clear timer as uppercase member variables")
		}
